...
from app.validators.forms import (DispatchAuth, DispatchAuths, NewGroup,
                                  ...
                                  UpdateUserInfoForm,UserUpdateInfoForm)
...
"""
@api {put} /cms/admin/user/update 更新用户信息
@apiName userUpdate 
@apiVersion 1.0.0
@apiGroup 用户模块
...
@apiParam {String} [confirm_password] 密码
@apiParam {String} [password] 确认密码
@apiParam {Number} group_id 分组ID
@apiUse Authorization
"""

@admin_api.route('/user/update', methods=['PUT'])
@route_meta(auth='禁用用户', module='管理员', mount=False)
@admin_required
def user_update():
    form = UserUpdateInfoForm().validate_for_api()
    user = manager.user_model.get(id=form.id.data)
    if user is None:
        raise NotFound(msg='用户不存在')
    ...
    with db.auto_commit():
        ...
        if form.username.data != user.username:
            exists = manager.user_model.get(username=form.username.data)
            if exists:
                raise ParameterException(msg='用户名已被注册，请重新输入用户名')
            user.username = form.username.data
        if form.email.data != user.email:
            user.email = form.email.data
        if form.group_id.data != user.group_id:
...
"""
@api {put} /cms/admin/dispatch/patch 更新用户信息
@apiName userUpdate 
@apiVersion 1.0.0
@apiGroup 用户模块
@apiParam {Number} id 用户ID
...
@admin_api.route('/remove/patch', methods=['POST'])
